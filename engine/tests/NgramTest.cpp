...
TEST_F(NgramTest, TestUnigram) {
    auto result =
        CandidateFinder::ContinuousSingleMatch(engine(), std::nullopt, "e5");
...
TEST_F(NgramTest, TestUnigramSort) {
    auto result = CandidateFinder::MultiMatch(engine(), std::nullopt, "e5");
    EXPECT_EQ(result[0].Text(), "个");
    EXPECT_EQ(result[1].Text(), "兮");
    EXPECT_EQ(result[2].Text(), "鞋");
    RecordUnigrams({"鞋"});
    result = CandidateFinder::MultiMatch(engine(), std::nullopt, "e5");

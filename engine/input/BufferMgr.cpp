...
class BufferMgrImpl : public BufferMgr {
   ...
   private:
    ...
    void BuildPreedit(proto::Preedit *preedit) override {
        {
            ...
            while (it != end) {
                if ((it != end - 1 && it->IsVirtualSpace() &&
                     !it[1].IsConverted()) ||
                    (!it->IsConverted())) {
                    auto composing_text = std::string();
                    while (it != end &&
                           (it->IsVirtualSpace() || !it->IsConverted())) {
                ...
                if (it->IsVirtualSpace()) {
                    ...
                    if (std::distance(m_composition.CBegin(), it) ==
                        m_focused_element) {
    ...
    void Erase(CursorDirection direction) override {
        ...
        auto pos_in_elem =
            m_caret - Buffer::TextSize(m_composition.Begin(), elem_it);
    ...
    void MoveFocusOrCaret(CursorDirection direction) {
        if (m_nav_mode == NavMode::BySegment && m_focused_element == 0 &&
            direction == CursorDirection::L) {
            m_nav_mode = NavMode::ByCharacter;
        }
    ...
    void FocusElement(Buffer::const_iterator element) {
        if (auto idx = static_cast<size_t>(
                std::distance(m_composition.CBegin(), element));
            idx != m_focused_element) {
    ...
    void UpdateCandidatesForFocusedElement() {
        ...
        m_candidates =
            CandidateFinder::MultiMatch(m_engine, FocusLGram(), raw_text);
    ...
    void JoinBufferUpdateCaretAndFocus(utf8_size_t raw_caret) {
        ...
        auto focus_raw_caret = m_composition.Empty()
                                   ? m_precomp.RawTextSize()
                                   : m_precomp.RawTextSize() + 1;
    ...
    void SetCompositionAndCandidatesContinuous(
        std::string const &raw_composition) {
        m_candidates = CandidateFinder::ContinuousMultiMatch(
            m_engine, FocusLGram(), raw_composition);
    ...
    void SetCompositionAndCandidatesBasic(std::string const &raw_composition) {
        m_candidates = CandidateFinder::MultiMatch(
            m_engine, FocusLGram(), raw_composition);
        ...
        if (!m_candidates.empty()) {
            ...
            if (raw_comp_size > top_cand_size) {
                m_composition.Append(BufferElement::Builder()
                                         .FromInput(raw_composition.substr(
                                             top_cand_size, raw_comp_size))
                                         .Build());
            }
        }
    ...
    void EraseConverted(
        BufferElementList::iterator caret_element, utf8_size_t caret) {
    ...
    size_t FocusCandidate_(size_t index, bool selected) {
        ...
        if (raw_buffer_size > raw_candidate_size) {
            ...
            auto deconverted = std::string(
                raw_text.begin() + static_cast<int>(raw_candidate_size),
                raw_text.end());

            while (it != end) {
                if (!CandidateFinder::HasExactMatch(m_engine, deconverted) ||
                    deconverted.empty()) {
            ...
            auto next_buf = CandidateFinder::ContinuousSingleMatch(
                m_engine, FocusLGram(), deconverted);

            if (next_buf.Empty()) {
                next_buf.Append(
                    BufferElement::Builder().FromInput(deconverted).Build());
            }
    ...
    void SelectCandidate_(size_t index) {
        ...
        if (m_edit_state == EditState::Selecting &&
            n_elems_remaining > candidate_size) {
    ...
    std::optional<TaiToken> FocusLGram() {
        if (!m_precomp.Empty() && m_focused_element == m_precomp.Size()) {
            ...
            // auto it = m_composition.Begin() +
            // static_cast<int>(m_focused_element) - 1;
